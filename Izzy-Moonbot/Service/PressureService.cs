...
public class PressureService
{
    ...
    public PressureService(LoggingService logger, ModLoggingService modLog, ServerSettings settings, ModService mod,
        ...
        SocketCommandContext context)
    {
        ...
        var pressure = _settings.SpamBasePressure;
        var pressureTrace = $"{_settings.SpamBasePressure} base, ";

        if (message.Attachments.Count >= 1 || message.Embeds.Count >= 1 || message.Stickers.Count >= 1)
        {
            // Image pressure.
            pressure += _settings.SpamImagePressure *
        ...
        {
            pressure += _settings.SpamPingPressure * _mention.Matches(message.Content).Count;
            pressureTrace +=
                $"{_settings.SpamPingPressure * _mention.Matches(message.Content).Count} mentions, ";
        }
        ...
        pressureTrace +=
            $"{_settings.SpamLinePressure * (message.Content.Replace("\r\n", "\n").Split("\n").Length - 1)} lines.";
        ...
        await _logger.Log($"Pressure increased by {pressure} to {newPressure}/{_settings.SpamMaxPressure}", context,
        ...
        if (newPressure >= _settings.SpamMaxPressure)
        {
            ...
            else
            {
                ...
                await _mod.SilenceUser(message.Author as SocketGuildUser, DateTimeOffset.Now, null,
                    $"Exceeded pressure max ({newPressure}/{_settings.SpamMaxPressure}) in <#{message.Channel.Id}>");
    ...
    public async Task ProcessMessageUpdate(IMessage oldMessage, SocketMessage newMessage)
    {
        if (_settings.SpamMonitorEdits)
            if (GetStringDifference(oldMessage.Content, newMessage.Content) >= _settings.SpamEditReprocessThreshold)
                // Reprocess
                if (_settings.SpamEnabled)
                    await ProcessPressure(newMessage.Author.Id, newMessage as SocketUserMessage,
                        newMessage.Author as SocketGuildUser, null);
    }
